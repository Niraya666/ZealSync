...
| **Hermes** | 存在 `~/.hermes/` 目录、支持 `memory` / `session_search` / `clarify` 工具 |
...
| **openClaw** | `~/.openclaw/workspace/USER.md` | 文件读取 | 已有的画像数据 |
| **Hermes** | `~/.hermes/memories/` 下的记忆文件 + 历史会话 | `memory` / `session_search` 工具 | 用户偏好、项目经验、兴趣、工作风格 |

**Hermes 特殊说明**：

Hermes 的 memory 系统通过工具访问，而非直接文件读取：
- 使用 `memory(action="read", target="user")` 读取用户 profile
- 使用 `memory(action="read", target="memory")` 读取环境记忆
- 使用 `session_search(query="...")` 搜索历史会话中的相关信息

...
description: Use when Codex needs to verify lark-cli, OAuth status, and Feishu account readiness before creating or submitting a ZealSync onboarding profile.
---

# ZealSync Onboarding — Init for Codex
...
在 Codex 中，安装 CLI、打开浏览器授权、写入用户主目录或运行需要网络的命令前，先说明目的并遵循当前会话的权限确认机制。

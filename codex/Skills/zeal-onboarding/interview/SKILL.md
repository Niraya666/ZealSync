...
description: Use when Codex needs to complete a ZealSync USER.md from a draft through concise natural-language follow-up questions, especially after extraction leaves missing profile sections.
---

# ZealSync Onboarding — Interview for Codex

## 职责

- 读取 `USER.md.draft`，对比完整模板，识别缺失维度
- 通过 Codex 自然对话分轮次补充信息
- 控制总轮数在 **5 轮以内**
- 输出完善的 `USER.md`
...
Codex 使用自然对话交互，Agent 直接向用户提问，用户直接回复。默认不要依赖 harness 专属问答工具；如果当前环境提供结构化提问工具，且问题是明确的 2-3 个选项，才可使用。

每轮最多提出 3 个问题；如某一 Section 已经充实，跳过该轮，避免重复打扰用户。
...
如用户给出模糊回答，先做合理归纳并请用户确认；只有影响匹配质量的关键信息（需求、供给、可用时间、隐私边界）才继续追问。

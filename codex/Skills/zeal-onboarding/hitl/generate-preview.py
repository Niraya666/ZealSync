...
def render_sections(body_md):
    ...
    for match in re.finditer(section_pattern, body_md, re.DOTALL | re.MULTILINE):
        ...
        sections.append(f'<div class="section"><h2>{title_html}</h2><div>{content_html}</div></div>')
...
def main():
    ...
    html_output = html_output.replace('{{NICKNAME}}', html.escape(nickname))
    html_output = html_output.replace('{{DESCRIPTION}}', html.escape(description))
    html_output = html_output.replace('{{TIMESTAMP}}', html.escape(timestamp))
    html_output = html_output.replace('{{TAGS_HTML}}', tags_html)
    html_output = html_output.replace('{{SECTIONS_HTML}}', sections_html)
    html_output = html_output.replace('{{RAW_MARKDOWN}}', html.escape(md_content))
    html_output = html_output.replace('{{SERVER_PORT}}', html.escape(actual_port))

...
description: Use when Codex needs to preview, edit, confirm, and submit a ZealSync USER.md profile through a local HTML HITL page and Lark document upload.
---

# ZealSync Onboarding — HITL for Codex

## 职责

- 读取完善的 `USER.md`
- 生成美观的 HTML 预览页面
- 优先用 Codex Browser 打开本地预览；不可用时给出 HTML 绝对路径
- 引导用户确认或提出修改
- 确认后上传至飞书文档
...
python3 ./codex/Skills/zeal-onboarding/hitl/save-server.py \
...
如系统无 Python，跳过 HTML 保存服务，直接把 `./USER-profile/{{nickname}}/USER.md` 的绝对路径发给用户，请用户用本机编辑器打开并保存后回到 Codex 对话中确认。不要无提示地调用 GUI `open` 命令。
...
```bash
python3 ./codex/Skills/zeal-onboarding/hitl/generate-preview.py \
  --server-log /tmp/zeal-server-{{nickname}}.log \
  --user-md ./USER-profile/{{nickname}}/USER.md \
  --template ./codex/Skills/zeal-onboarding/hitl/hitl-template.html \
...
Codex 优先使用 Browser 插件打开本地预览：

- 若当前会话可用 Browser / browser-use，导航到 `file:///ABSOLUTE_PATH/USER-profile/{{nickname}}/hitl-preview.html`
- 若 Browser 不可用，返回 `hitl-preview.html` 的绝对路径，请用户手动打开
- 如果必须使用系统 GUI `open` 命令，先说明原因并通过当前 Codex 会话的权限确认机制执行

### 4. 等待用户确认

在对话中询问：

> 已生成你的画像预览，底部提供了 Markdown 编辑区。
>
> **操作方式**：
> 1. 在浏览器中直接编辑，点击"保存"自动覆盖 `USER.md`
> 2. 回到 Codex 对话中回复 **"已保存"** 或 **"确认提交"**
...
这是 `lark-cli docs +create --content @USER-lark.md` 的已知限制；其他文件操作仍优先使用绝对路径。
...
- Codex 权限限制 → 需要联网、OAuth 或 GUI 操作时，按当前会话权限机制请求用户确认

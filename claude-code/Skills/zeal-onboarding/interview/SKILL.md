...
description: ZealSync onboarding 对话式问答。基于已有草稿对比模板，通过 AskUserQuestion 在5轮内完善 USER.md。
---
...
每轮必须提供"跳过此轮"的选项。用户跳过的 Section 保留占位符，在最终 USER.md 中标记为 `[待补充]`。
...
_Profile version, generation info, data lineage, confirmation status, freshness, and visibility rules._

- Version: v1.0.0
- Generated: {{timestamp}}
- Sources: [onboarding interview, {{memory sources}}, {{external links}}]
